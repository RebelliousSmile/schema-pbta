...
1. Track Lantern #20 as the remaining host prerequisite. Lantern must first commit its candidate URL and SRI-bearing active pnpm lock on a dedicated adoption branch. Because a frozen Lantern install resolves all three direct schema archives, that same commit must normalize the existing-version Mist and Adrenaline lock entries to stable URLs and SRI too; it must not upgrade their versions. Lantern then provides that immutable commit to the train; #49 supplies Handbook's proof foundation and #52 (Handbook v2.26.0) supplies its canonical manifest/evidence interface. Do not replace their adapters or renderers from schema-pbta.
